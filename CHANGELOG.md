...
- **The head unit can now be asked what it publishes.** `RuntimeInterfaceProbe` walks the
  ...
  this file exists not to do. There is a second hub behind engineering mode, mapped the same
  way, and the survey asks it for the four part numbers the head unit states about itself —
  identity the vehicle asserts, where the generation label is something the project inferred.
  It also reads the second speed source, which reports a float where the source the write gate
  trusts is integral. Nothing in production calls any of it; it exists to be run once from an
  unstable capture and turned into answers.
...
- **A generated catalogue document.** `docs/catalogue.md` lists every `ConditionType` and every
  ...
  the generations it is declared on. Rendered from the enums by `CatalogueDocTest`, which
  rewrites it on every test run exactly as `FirmwareSupportTest` does for the matrix — the
  hand-written list it replaces had gone a year without mentioning twenty entries and still
  promised an "AM or FM only" tune action.
